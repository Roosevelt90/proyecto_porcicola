                ...
                <td>  
                  <?php if ($key->$estado == true): ?>
                    <?php if (session::getInstance()->hasCredential('admin') == 1): ?>
                      ...
                      <a id="insertDetalle<?php echo $countDetale ?>" href="#myModalDetail<?php echo $key->$id ?>" class="btn btn-sm btn-primary fa fa-bars" data-toggle="modal" data-target="#myModalDetail<?php echo $key->$id ?>" class="btn btn-info btn-xs"></a>
                ...
                <form id="detailForm" class="form-horizontal" method="POST" action="<?php echo routing::getInstance()->getUrlWeb('factura', 'createDetalleFacturaVenta') ?>">
                  ...
                  <select name="<?php echo detalleProcesoVentaTableClass::getNameField(detalleProcesoVentaTableClass::ANIMAL, true) ?>">
                    ...
                    <?php foreach ($objAnimal as $key): ?>
                      <option value="<?php $key->id ?>"><?php echo $key->numero_identificacion ?></option>
                    <?php endforeach; ?>
                  </select>
                  Valor
                  <input name="<?php echo detalleProcesoVentaTableClass::getNameField(detalleProcesoVentaBaseTableClass::VALOR, true) ?>">
                  <input value="<?php echo $key->$id ?>" type="hidden" name="<?php echo detalleProcesoVentaTableClass::getNameField(detalleProcesoVentaBaseTableClass::VENTA, true) ?>">
                  <br/>
                  <br/>
                  <div class="modal-footer">
                  <a href="#close2" title="Close" class="close2 btn btn-default fa fa-times-circle-o close2"> <?php echo i18n::__('cancel') ?></a>
                  <button type="submit" class="btn btn-primary fa fa-ban"   > <?php echo i18n::__('register') ?></button>
                  </div>
                </form>
              </div>
            </div>
